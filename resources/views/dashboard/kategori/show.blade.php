@extends('dashboard.layout.main')

@section('container')
      <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">Detail Kategori</h1>
      </div>
<div class="d-flex justify-content-center">
<div class="card" style="width: 30rem;">
  @if($kategori->image)
    <img src="{{ asset('storage/' . $kategori->image) }}" class="card-img-top" width="500" height="500">
  @else
  <img src="/image/none.png" class="card-img-top" alt="...">
  @endif
  <ul class="list-group list-group-flush">
    <li class="list-group-item">Kode Kategori : {{ $kategori->kode_kategori}}</li>
    <li class="list-group-item">Nama Kategori : {{ $kategori->nama_kategori}}</li>
    <li class="list-group-item">Slug Kategori : {{ $kategori->slug}}</li>
    <li class="list-group-item">{!! $kategori->deskripsi_kategori !!}</li>
  </ul>
  <div class="card-body">
    <a href="/dashboard/kategori" class="card-link btn btn-success"><span data-feather="arrow-left"></span>Kembali</a>
    <a href="/dashboard/kategori/{{$kategori->slug}}/edit" class="btn btn-warning"><span data-feather="edit"></span> Edit</a>
    <form action="/dashboard/kategori/{{ $kategori->id }}" method="post" class="d-inline">
      @method('delete')
      @csrf
      <button class="btn btn-danger"  onclick="return confirm('Apakah Yakin ingin Menghapus Data?')">
        <span data-feather="x-circle"></span>Hapus</button>
    </form>
  </div>
</div>
</div>
@endsection